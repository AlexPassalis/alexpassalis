  ...
  const mutation = useMutation({
    mutationFn: async ({ email }: { email: string }) => {
      const response = await axios.post(
        `${env.NEXT_PUBLIC_FASTIFY_ORIGIN}/auth/signup/email`,
        { email },
        {
          headers: {
    ...
    <div className={cn('flex flex-col gap-6', className)} {...props}>
      <Card>
        <CardHeader className="text-center">
          ...
          <CardDescription>
            Sign up with your Google or Apple account
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form
            onSubmit={async e => {
              ...
              mutation.mutate({ email: emailValidated })
            }}
          >
            <div className="grid gap-6">
              <div className="flex flex-col gap-4">
                <Button
                  type="button"
                  onClick={async () => {
                    router.push('https://localhost/api/auth/google')
                  }}
                  variant="outline"
                  className="w-full"
                >
